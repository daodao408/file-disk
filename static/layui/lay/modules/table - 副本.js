    ...
    j.prototype.config = {
                ...
                a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)),
                    t.ajax({
                        ...
                        error: function(e, t) {
                            if (e.status == '401') {
                                // alert("没有登陆,请登陆!")
                                setTimeout(window.location = '/index', 2);
                            } else if (e.status == '400' && e.responseJSON.message.indexOf("删除,请刷新") > -1) {
                                i.layMain.html('<div class="' + h + '">操作的' + e.responseJSON.message + '</div>');
                            } else if (e.status == '400' && e.responseJSON.message.indexOf("已取消,无法操作该文件,请刷新界面") > -1) {
                                i.layMain.html('<div class="' + h + '">操作的' + e.responseJSON.message + '</div>');
                            } else {
                                i.layMain.html('<div class="' + h + '">数据接口请求异常</div>');
                                i.renderForm();
                                i.setColsWidth();
                            }
                        }
        ...
        j.prototype.events = function() {
            ...
            a.layBody.on("change", "." + S, function() {
                ...
                //     a.layBody.on('mouseout', '.layui-table', function() {
                //         layer.close(a.tipsIndex);
                //     })

                // a.layBody.on('mouseover', 'td', function() {
                //     var othis = t(this),
                //         field = othis.data('field'),
                //         editType = othis.data('edit'),
                //         elemCell = othis.children(f);
                //     layer.close(a.tipsIndex);
                //     // if (othis.attr('data-content') != "") {
                //     //     return
                //     // }
                //     if (othis.data('off')) return;
                //     if (Math.round(elemCell.prop('offsetWidth ')) < Math.round(elemCell.prop('scrollWidth'))) {
                //         a.tipsIndex = layer.tips(elemCell.html(), elemCell[0], {
                //             tips: [1, '#333'],
                //             time: -1,
                //             anim: -1,
                ...
                //                 layero.find('.layui-table-tips').on('mouseout', function() {
                //                     layer.close(index);
                //                 });
                //             }
                //         });
            ...
            a.layBody.on("click", "." + g, function() {
                    ...
                    a.tipsIndex = l.tips(['<div class="layui-table-tips-main" style="margin-top: -' + (n.height() + 16) + "px;" +
                        ...
                        }() + '">', n.html(), "</div>", '<i class="layui-icon layui-table-tips-c layui-icon-close"></i>'
                    ].join(""), n[0], {
                        tips: [3, ""],
                        time: -1,
                        anim: -1,
